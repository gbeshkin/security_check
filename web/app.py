    ...
    for report_dir in sorted(REPORTS_ROOT.iterdir(), reverse=True):
        ...
        try:
            ...
            reports.append({
                ...
                "severity_totals": report_data.get("severity_totals", {}),
            })
        except Exception:
            continue
    return reports
...
def save_meta(output_dir: Path, source_type: str, source_value: str, scanner_result):
    meta = {
        "created_at": datetime.utcnow().isoformat() + "Z",
        "source_type": source_type,
        "source_value": source_value,
        "scanner_returncode": scanner_result.returncode,
        "scanner_stdout": scanner_result.stdout,
        "scanner_stderr": scanner_result.stderr,
    }
...
def scan_github():
    ...
    if not repo_url:
        return "Нужно указать GitHub URL", 400

    if not is_valid_github_repo_url(repo_url):
        return "Поддерживаются только полные ссылки вида https://github.com/owner/repo", 400
    ...
    clone_result = subprocess.run(
        ["git", "clone", "--depth", "1", f"{repo_url}.git", str(temp_repo_dir)],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
        check=False,
...
def scan_local():
    ...
    display_name = request.form.get("display_name", "").strip() or "local-project"

    if not local_path:
        return "Нужно указать путь к проекту", 400

    scan_target = Path(local_path).expanduser().resolve()
    if not scan_target.exists():
        return f"Путь не найден: {scan_target}", 400

    scan_id = str(uuid.uuid4())
    output_dir = REPORTS_ROOT / scan_id
    output_dir.mkdir(parents=True, exist_ok=True)

    result = run_scanner(scan_target, display_name, output_dir)
    save_meta(output_dir, "local", str(scan_target), result)
