    ...
    try:
        ...
        if clone_result.returncode != 0:
            ...
            (output_dir / "error.txt").write_text(
                clone_result.stderr or clone_result.stdout or "Clone failed",
                encoding="utf-8"
            )
            return render_template(
                "scan_error.html",
                repo_url=repo_url,
                error=clone_result.stderr or clone_result.stdout
            ), 400
        ...
        (output_dir / "error.txt").write_text("Scan timed out.", encoding="utf-8")
        return render_template(
            "scan_error.html",
            repo_url=repo_url,
            error="The scan took too long and timed out."
        ), 500
