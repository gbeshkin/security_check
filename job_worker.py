...
def process_job(job_path: Path):
    ...
    try:
        ...
        if clone.returncode != 0:
            ...
            save_json(output_dir / "meta.json", {"created_at": utc_now(), "source_type": "github", "source_value": repo_url, "scanner_returncode": 1, "scanner_stdout": clone.stdout, "scanner_stderr": clone.stderr})
        ...
        save_json(output_dir / "meta.json", {"created_at": utc_now(), "source_type": "github", "source_value": repo_url, "scanner_returncode": result.returncode, "scanner_stdout": result.stdout, "scanner_stderr": result.stderr})
        update_status(job_path, "completed" if result.returncode in (0,2) else "failed", finished_at=utc_now())
